...
Правила:
- Каждый вопрос — короткая реальная ситуация из жизни этого бизнеса, по-доброму, на «ты». 3–4 варианта.
- КАЖДЫЙ вариант — конкретное реальное действие, как его описал бы сам человек, а НЕ абстрактная «займусь маркетингом». Эталон вопроса:
  «Месяц затишья в заказах. Что делаешь?» →
   • снимаю видео и фото, оживляю Instagram и TikTok [marketing]
   • пишу старым клиентам, зову вернуться [operations]
   • копаюсь в рекламе — кому её вообще показывают [marketing]
   • пересобираю, чем я отличаюсь от других [brand]
- Оси: product (Продукт), marketing (Маркетинг), operations (Операционка), brand (Бренд). Внутри одной ситуации оси МОГУТ повторяться, если это РАЗНЫЕ конкретные действия (например два разных маркетинговых хода) — так мы понимаем не только ЧТО, но и КАК человек растёт.
- МАРКЕТИНГ И ПРИВЛЕЧЕНИЕ раскрывай щедро и по граням (контент/охват, реклама и аудитория, возвращение клиентов, повод выбрать именно тебя) — это самая частая слепая зона, не своди его к одному варианту.
- Баланс: каждая из 4 осей должна встречаться как вариант минимум в 3–4 вопросах из 7 — чтобы вывод был честным, а не следствием пропущенной оси.
- КАЖДЫЙ вариант — валидное поведение (нет «глупых» и «правильных»), человек выбирает самый честный. Не намекай, какой ответ лучше.
- situation — до 100 знаков, живым тёплым языком. Вариант — до 70 знаков, конкретный однозначный ответ.

${PLAIN_LANGUAGE_RULE}

Верни ТОЛЬКО JSON: {"questions":[{"situation":"...","options":[{"text":"...","axis":"ключ оси"},{"text":"...","axis":"ключ оси"},{"text":"...","axis":"ключ оси"}]}]}`;
}
...
export async function runGeneratorSituational(
  ...
  // Разрешаем несколько граней одной оси в вопросе (не дедупим по оси). Берём валидные
  // варианты (3–4), баланс осей по всему набору обеспечивает промпт.
  return (res.questions ?? [])
    ...
    .map((q, i) => ({
      ...
      options: (q.options ?? [])
        ...
        .map((o) => ({ text: String(o.text), axis: o.axis }))
        .slice(0, 4),
    }))
